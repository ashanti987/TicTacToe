...
class PygameVisualizer:
    def __init__(self):
        ...
        pygame.display.set_caption("Tic-Tac-Toe AI Comparison - Human vs AI")
        ...
        self.visualization_data = {
            ...
            'current_move': None,
            'total_nodes': 0,   # Track cumulative nodes across moves
            'total_time': 0.0   # Track cumulative time across moves
        }
    ...
    def draw_board(self):
        ...
        if self.visualization_data['current_move'] is not None:
            ...
            if self.visualization_data['current_algorithm'] == 'BFSAI':
                color = GREEN
            elif self.visualization_data['current_algorithm'] == 'GreedyAI':
                color = ORANGE
            else:  # MinimaxAI
                color = PURPLE
    ...
    def draw_algorithm_info(self):
        ...
        # Algorithm buttons
        algorithms = [
            ("Breadth-First Search", self.bfs_ai, GREEN),
            ("Greedy Best-First", self.greedy_ai, ORANGE),
            ("Minimax Alpha-Beta", self.minimax_ai, PURPLE)
        ]
        ...
        for i, (name, ai, color) in enumerate(algorithms):
            button_y = panel_y + 60 + i * (button_height + button_spacing)
            btn_color = color if self.current_ai == ai else GRAY
    ...
    def draw_metrics(self):
        ...
        # Current algorithm
        algo_name = "Human vs Human" if self.current_ai is None else self.current_ai.__class__.__name__
        ...
        nodes_text = self.small_font.render(
            f"Last Move - Nodes: {self.visualization_data['nodes_evaluated']:,}", True, BLACK)
        time_text = self.small_font.render(
            f"Last Move - Time: {self.visualization_data['computation_time']:.4f}s", True, BLACK)
        
        # Cumulative metrics
        total_nodes_text = self.small_font.render(
            f"Total Nodes: {self.visualization_data['total_nodes']:,}", True, BLACK)
        total_time_text = self.small_font.render(
            f"Total Time: {self.visualization_data['total_time']:.4f}s", True, BLACK)
        
        self.screen.blit(nodes_text, (metrics_x + 20, metrics_y + 80))
        self.screen.blit(time_text, (metrics_x + 20, metrics_y + 105))
        self.screen.blit(total_nodes_text, (metrics_x + 20, metrics_y + 130))
        self.screen.blit(total_time_text, (metrics_x + 20, metrics_y + 155))
    
    def draw_game_info(self):
        """Draw game state information and instructions."""
        info_x = MARGIN
        info_y = BOARD_SIZE + MARGIN * 2 + 190
        
        # Game status
        status = "Game Over - " if self.game.game_over else ""
        if self.game.winner:
            status += f"Winner: {self.game.winner} ({'Human' if self.game.winner == 'X' else 'AI'})"
        elif self.game.game_over:
            status += "Draw"
        else:
            player_type = "Human" if self.game.current_player == 'X' else "AI"
            status += f"Current Player: {self.game.current_player} ({player_type})"
        
        status_text = self.font.render(status, True, BLACK)
        self.screen.blit(status_text, (info_x, info_y))
        
        # Game summary when over
        if self.game.game_over and self.visualization_data['current_algorithm']:
            summary = f"Final - {self.visualization_data['current_algorithm']}: " \
                     f"{self.visualization_data['total_nodes']:,} nodes, " \
                     f"{self.visualization_data['total_time']:.4f}s total"
            summary_text = self.small_font.render(summary, True, BLACK)
            self.screen.blit(summary_text, (info_x, info_y + 40))
        
        # Simple instructions below game status
        instructions = [
            "Click buttons to select AI • Click board to play as X (Human)",
            "Keyboard: 1=BFS, 2=Greedy, 3=Minimax • Space=Auto-play • R=Reset"
        ]
        
        for i, instruction in enumerate(instructions):
            instruction_text = self.tiny_font.render(instruction, True, BLACK)
            self.screen.blit(instruction_text, (info_x, info_y + 80 + i * 20))
    ...
    def handle_click(self, pos):
        ...
        if (MARGIN <= x <= MARGIN + BOARD_SIZE and 
            ...
            # Make human move (Human always plays as X)
            if not self.game.game_over and self.game.current_player == 'X':
                success = self.game.make_move(position)
                if success:
                    self.move_count += 1
                    print(f"\n--- Move {self.move_count} ---")
                    print(f"Human (X) played at position {position}")
                    print(f"Board after human move:")
                    self._print_board_ascii()
                    
                    # If game continues, let AI make a move as O
                    if not self.game.game_over and self.current_ai is not None:
                        print(f"\n{self.current_ai.__class__.__name__} is thinking...")
                        self.make_ai_move()
                    elif self.game.game_over:
        ...
        for i in range(3):
            ...
            if (panel_x + 10 <= x <= panel_x + 260 and 
                ...
                algo_name = algorithms[i].__class__.__name__
                print(f"\n=== Selected {algo_name} as AI opponent ===")
                
                # Reset cumulative metrics when switching algorithms
                self.visualization_data['total_nodes'] = 0
                self.visualization_data['total_time'] = 0.0
                self.move_count = 0
                
                # If it's AI's turn and we just selected an algorithm, make AI move
    ...
    def make_ai_move(self):
        ...
        if self.current_ai and not self.game.game_over:
            ...
            if move is not None:
                ...
                self.visualization_data['total_nodes'] += nodes
                self.visualization_data['total_time'] += comp_time
                self.move_count += 1
                ...
                print(f"  Cumulative: {self.visualization_data['total_nodes']:,} nodes, "
                      f"{self.visualization_data['total_time']:.4f}s total")
    ...
    def _print_game_result(self):
        """Print final game result with summary."""
        if self.game.winner:
            winner_type = "Human" if self.game.winner == 'X' else "AI"
            print(f"\n*** GAME OVER ***")
            print(f"Winner: {self.game.winner} ({winner_type})")
        else:
            print(f"\n*** GAME OVER ***")
            print(f"Result: Draw")
        
        if self.visualization_data['current_algorithm']:
            print(f"Algorithm: {self.visualization_data['current_algorithm']}")
            print(f"Total nodes evaluated: {self.visualization_data['total_nodes']:,}")
            print(f"Total computation time: {self.visualization_data['total_time']:.4f}s")
            print(f"Average nodes per move: {self.visualization_data['total_nodes'] / self.move_count:,.1f}")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_r:  # Reset game
                        ...
                        mode = "ON" if self.auto_play else "OFF"
                        print(f"\nAuto-play mode: {mode}")
                    elif event.key in [pygame.K_1, pygame.K_2, pygame.K_3]:  # Select AI
                        algorithms = [self.bfs_ai, self.greedy_ai, self.minimax_ai]
                        self.current_ai = algorithms[event.key - pygame.K_1]
                        algo_name = algorithms[event.key - pygame.K_1].__class__.__name__
                        print(f"\n=== Selected {algo_name} as AI opponent ===")
            
            # Auto-play mode (AI vs AI)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("GAME MODE: Human (X) vs AI (O)")
    print("=" * 60)
    print("\nInstructions displayed on screen • Detailed analysis in terminal")
    print("\nStarting new game... Human (X) goes first!")
    print("=" * 60)
